...
auto Web::on_wheel(int, const EmscriptenWheelEvent *event, void *user_data) -> bool {
    auto *app = static_cast<Web *>(user_data);
    if (app != nullptr && event != nullptr) {
...
auto Web::on_key_down(int, const EmscriptenKeyboardEvent *event, void *user_data) -> bool {
    auto *app = static_cast<Web *>(user_data);
    if (app != nullptr && event != nullptr) {
...
void Web::push_key_down_event(const EmscriptenKeyboardEvent &event) {
    ...
    if (key == Key::kNone) {
        ...
        if (event.key[1] == '\0' && c >= 32 && c <= 126 && !event.ctrlKey && !event.metaKey) {
            InputEvent text = {};
...
auto Web::make_pipeline() -> b8 {
    ...
    layer_layout_entries[1].texture.viewDimension = wgpu::TextureViewDimension::e2DArray;
    ...
    layer_bind_group_layout_ = device_.CreateBindGroupLayout(&layer_layout_descriptor);
    ...
    composite_descriptor.primitive.topology = wgpu::PrimitiveTopology::TriangleList;
...
auto Web::make_layer_texture() -> b8 {
    ...
    binding_dimension.textureBindingViewDimension = wgpu::TextureViewDimension::e2DArray;
...
auto Web::upload_draws() -> b8 {
    ...
    for (usize index = 0; index < gui_.layers.size() && index < layer_rows.size(); ++index) {
...
void Web::encode_layer_clears(wgpu::CommandEncoder encoder) {
    for (const u8 slot : gui_.clear_slots.span()) {
        if (slot >= layer_slice_views_.size() || layer_slice_views_[slot] == nullptr) {
            continue;
        }
        wgpu::RenderPassColorAttachment attachment = {};
