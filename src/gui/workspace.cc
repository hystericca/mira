...
void zoomat(GuiState *state, i32 x, i32 y, i32 wheel_delta) {
  ...
  state->view.x =
      document_x -
      ((static_cast<f32>(x) - state->layout.viewport.x) / state->view.zoom);
  state->view.y =
      document_y -
      ((static_cast<f32>(y) - state->layout.viewport.y) / state->view.zoom);
}
...
[[nodiscard]] Rect stampviewrect(const GuiState &state, f32 document_x,
                                 f32 document_y, Brush spec) {
...
[[nodiscard]] PaintStamp stampfor(f32 document_x, f32 document_y, Brush spec,
                                  f32 layer_slot) {
  return {
      .x = document_x,
      .y = document_y,
...
[[nodiscard]] b8 emitrect(f32 x0, f32 y0, f32 x1, f32 y1, Emit emit) {
...
[[nodiscard]] b8 draftmark(GuiState *state, f32 document_x, f32 document_y,
                           ToolKind kind) {
  if (!painttool(kind) || !in_document(*state, document_x, document_y)) {
    return true;
  }
  return state->draft_stamps.push(
      stampfor(document_x, document_y, brushfor(*state, kind),
               static_cast<f32>(state->draft_layer_slot)));
}

[[nodiscard]] b8 draftline(GuiState *state, f32 x0, f32 y0, f32 x1, f32 y1,
                           ToolKind kind) {
...
[[nodiscard]] b8 draftrect(GuiState *state, f32 x0, f32 y0, f32 x1, f32 y1,
                           ToolKind kind) {
...
void draftstart(GuiState *state, const Layer &layer, ToolKind kind, f32 x,
                f32 y) {
...
void historymark(GuiState *state, PaintStamp stamp) {
  ...
  state->active_stroke_rect =
      state->active_stroke_count == 0
          ? bounds
          : unionrect(state->active_stroke_rect, bounds);
...
void historyreplay(GuiState *state) {
  ...
  for (const Stroke action : state->strokes.span()) {
    if (action.stamp_count == 0 ||
        action.first_stamp >= state->history_stamps.size()) {
      continue;
    }
  ...
  for (u8 slot{0}; slot < kMaxLayers; ++slot) {
    for (usize action_index{0}; action_index < state->stroke_cursor;
         ++action_index) {
      const Stroke action = state->strokes[action_index];
      for (u32 stamp_index{0}; stamp_index < action.stamp_count;
           ++stamp_index) {
...
void workmove(GuiState *state, i32 x, i32 y, u8 buttons) {
  ...
  const f32 document_x{screen_to_paint_x(*state, x)};
  const f32 document_y{screen_to_paint_y(*state, y)};
  if (!markline(state, state->last_paint_x, state->last_paint_y, document_x,
                document_y, kind)) {
    state->painting = false;
    return;
  }
...
void workup(GuiState *state, i32 x, i32 y) {
  ...
  if (contains(state->layout.viewport, x, y)) {
    const f32 document_x{screen_to_paint_x(*state, x)};
    const f32 document_y{screen_to_paint_y(*state, y)};
    (void)markline(state, state->last_paint_x, state->last_paint_y, document_x,
                   document_y, kind);
  }
  historyfinish(state);
}
...
void workdraw(const GuiState &state, DrawList *draws) {
  ...
  if (state.hot_kind == HitKind::kViewport && painttool(active_tool) &&
    ...
    if (layer != nullptr && in_document(state, document_x, document_y) &&
      ...
      (void)add_preview_stamp(draws,
                              stampfor(document_x, document_y, spec,
                                       static_cast<f32>(layer->layer_slot)));
    }
  }

  const Rect visible_document =
      intersectrect(state.layout.viewport, state.layout.document);
  ...
  drawrect(draws,
           {.x = state.layout.viewport.x,
            ...
            .height = 1.0F},
           Tone::kBlack);
  drawtext(draws, "document", state.layout.viewport.x + 6.0F,
           state.layout.viewport.y + 5.0F, Tone::kBlack);
  if (!empty(visible_document)) {
